...
public class NetworkUtil {
	...
	public static boolean isConnected() {
		ConnectivityManager cm = (ConnectivityManager) AppThrower.getAppThrowerApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE);		
		try {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				if(!connected) {
					Toast.makeText(AppThrower.getAppThrowerApplicationContext(), "No network connection to sync", Toast.LENGTH_LONG).show();
				}				
				Looper.loop();
			}
		}.start();
	...
	public static boolean isConnectedWiFi() {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				if(!connectedWifi) {
					Toast.makeText(AppThrower.getAppThrowerApplicationContext(), "No WiFi connection to sync", Toast.LENGTH_LONG).show();
				}				
				Looper.loop();
			}
		}.start();
